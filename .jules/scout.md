...
## 2026-06-13 - [Semantic Upgrades for Chart Titles]
**Learning:** When generating multiple charts or plots inside a grid layout (like polar cuts), replacing generic `<div>` title wrappers with semantic heading tags (like `<h3>`) helps establish a clearer document outline for screen readers and crawlers. To ensure this doesn't disrupt the visual layout, you can apply inline styles such as `margin: 0, fontWeight: 'normal', fontSize: 'inherit'` or a specific font size to override the browser's default heading styles, satisfying both SEO and UI requirements.
**Action:** Always look for grouped graphical elements with pseudo-headings in SPAs, and upgrade them to semantic heading tags while strictly preserving their CSS layout with inline style resets.
## 2026-06-21 - [Semantic HTML Upgrade for PropagationRadar]
**Learning:** When upgrading a generic `<div>` to a semantic `<figure>` to logically group graphics (like an SVG radar plot) and their `<figcaption>`, browsers apply default block margins to `<figure>` that can cause visual layout shifts. To preserve the exact visual design without modifying CSS classes, explicitly add `margin: 0` to the inline style.
**Action:** Always verify browser default styles for semantic elements like `<figure>`, `<h3>`, and `<details>` when making SEO upgrades, and use targeted inline style resets (e.g., `margin: 0`, `fontSize: "inherit"`) to ensure the changes remain visually invisible.
