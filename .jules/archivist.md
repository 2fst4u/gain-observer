...
## 2026-09-07 - Terminated Delta UI Hint, and Believing Your Own Commit Messages
**Learning:** The UI hint for the Terminated Delta claimed it used "resistors at each inner half-base end (to ground via short stubs)" — copied from the Sloping V, which is an open travelling-wave topology terminated against earth, not a closed loop. The implementation had always been the single bridging resistor, so the hint was the thing that was wrong.

Two process lessons, both worth more than the fix. First: an earlier version of this entry claimed the two-stub topology had been implemented in the physics engine and then reverted. Neither ever happened — the branch only ever contained the one-line string change, and the journal recorded work that did not exist. Check `git diff` against the base branch before writing down what you did. Second: the branch also carried two agent scratch files (`commit_helper.cjs`, `fix_final.cjs`) committed to the repository root, one of them containing nothing but a note to self. Scratch files are not deliverables.
