...
## 2026-09-02 - Terminated Delta Physical Implementation & UI Clarification
**Learning:** The UI hint for the Terminated Delta incorrectly claimed it used "resistors at each inner half-base end (to ground via short stubs)" — a copy-paste error from the Sloping V logic that describes an open travelling-wave topology, rather than a closed loop. A previous review suggested updating the *physics* to match this incorrect description. Doing so broke the loop termination properties (current ripple, broadband flat impedance) because in NEC-2, terminating stubs without a true ground connection leaves them floating, and open stubs fail to properly terminate a loop anyway.
**Action:** Reverted the flawed attempt to change the physical topology to two grounded stubs. Restored the physically correct `TERMINATED_DELTA_BRIDGE_TAG` single-resistor bridge topology in the simulation engine. Then updated the `GeometryControl.tsx` UI string (again) to correctly describe this bridging resistor, aligning the documentation with the correct physical reality.
