...
## 2024-05-18 - RegExp Loop over Large Outputs vs Memory Rule Caveat
**Learning:** The memory rule to "avoid `text.split('\n')` by using a global, multiline regular expression (`/pattern/gm`) loop with `regex.exec(text)`" offers a massive speedup (~6x reduction in parse times) by eliminating array allocation overhead, BUT applying it directly on extremely large outputs (like 100MB+ NEC output logs) can backfire severely if the search bounds aren't controlled. If the expected match is missing or distant, `regex.exec(text)` will scan forward unconstrained, causing performance to plummet (up to 100x slower due to backtracking and scanning across gigabytes of combined data in sweeps).
**Action:** When applying the `/gm` + `regex.exec` pattern to localized blocks within huge strings, always bound the search. Use `text.indexOf` to find the block limits, then apply the regex loop either against `text.substring(blockStart, blockEnd)` (which is very cheap to allocate) or use a strict loop boundary `if (m.index > blockEnd) break;` for the master text string.
