...
## 2024-05-18 - String split optimization on large text structures
**Learning:** In text parsing of heavily repeated long strings like `.nout` NEC outputs (e.g., >65k lines), using `String.prototype.split('\n')` on the entire block allocates thousands of intermediate string objects, creating massive garbage collection overhead and significantly slowing down the `parseNecParser.ts` loops. Global `RegExp.exec()` instances (`/gm`) configured to resume via `lastIndex` or tightly bounded via iterative `indexOf('\n')` searches can execute in exactly a fraction of the time without generating useless intermediary string arrays.
**Action:** When extracting repetitive block data or sweeping tables from large raw API returns or external files, avoid large `split()` and `slice()` operations. Prefer `exec()` across bounded string windows or utilize `lastIndex` stateful matches to skip garbage collection overhead.
