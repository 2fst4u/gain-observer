...
## 2024-05-18 - Optimized String Parsing
**Learning:** Using `split('\n')` on large strings in JavaScript creates massive arrays of string slices, leading to significant garbage collection pressure and slow execution times. This is particularly relevant when parsing large text outputs like NEC results containing tens of thousands of lines.
**Action:** Instead of slicing and splitting strings, use O(1) windows (e.g. `text.slice(start, start + 1000)`) combined with the `/m` multiline flag in Regex, or use iterative zero-copy scans with `RegExp.exec()` and the `/gm` flag. Set `regex.lastIndex` directly to start searching at specific points in a large string.
