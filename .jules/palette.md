...
## 2024-05-20 - Dynamic Disabled Tooltips

**Learning:** For interactive UI elements (like a button to turn off termination resistance) that become disabled when a specific state is reached (e.g., when termination is already off), static `title` tooltips can be confusing or redundant.
**Action:** Always conditionally render the `title` attribute for disabled buttons so that the tooltip explicitly explains _why_ the button is disabled, rather than simply restating the button's action. This provides clear feedback and improves accessibility.
