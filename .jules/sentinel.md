...
## 2026-07-19 - Error Message Information Leakage
**Vulnerability:** Raw error messages (e.g. `error.message`) were exposed directly to the end user in the `ErrorBoundary` and `sceneFallback` components. While this is a client-side app, this can leak internal state, file paths, or sensitive logic details depending on what threw the exception.
**Learning:** Even in client-side React applications, displaying unhandled exceptions directly in the UI can inadvertently leak implementation details to users. Error boundaries should always fail securely.
**Prevention:** Conditionally check the environment (e.g., `import.meta.env.DEV`) before rendering detailed error information. Display a generic 'unexpected error' message in production environments.
## 2026-07-20 - Error Message Information Leakage (Solver Engine)
