...
- **Recommended value: `FOLDED_DIPOLE_FEED_R_OHMS` = 300 Ω** = $R_{feed}$, i.e. the 50 % / −3 dB point — the least that can be paid for broadband behaviour. **Not** the two-wire line's $Z_0$ (≈ 684 Ω at the default aperture), which was the earlier recommendation: that line is the *non-radiating* transmission-line mode, so terminating it in its own $Z_0$ flattens SWR without contributing to radiation, and measured 5.3 dB down at the design frequency. Across 7–28 MHz the 300 Ω + 9:1 pair holds the same worst-case SWR (5.6:1) as the old 684 Ω + 6:1 pair while returning 2.2 dB at the design frequency.
