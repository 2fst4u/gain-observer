...
export class Nec2Engine implements Engine {
  ...
  private async adaptiveSweep(
    ...
    // ⚡ Bolt: Removed intermediate arrays entirely. findSwrBands now accepts
    // the generic scan array and accessor functions directly.
    const primaryBands = findSwrBands(scan, (p) => p.frequencyMHz, effSwr, 2);
    ...
    if (!reachedLimits) {
      ...
      // ⚡ Bolt: Use findSwrBands accessors instead of intermediate arrays
      const broadBands = findSwrBands(broadScan, (p) => p.frequencyMHz, effSwr, BROAD_THRESHOLD);
